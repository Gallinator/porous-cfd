...
def evaluate(args, model, data: FoamDataset, enable_timing,
             sample_process_fn: Callable[[FoamDataset, FoamData, FoamData], tuple],
             postprocess_fn: Callable[[FoamDataset, list, Path], None]):
    ...
    for predicted, target in zip(predictions, data_loader):
        pde = FoamData(predicted, model.predicted_labels, target.domain)
        for i, r in enumerate(sample_process_fn(data, pde, target)):
            if len(results) == i:
                results.append([*r])
            else:
                results[i].extend(r)
